test_that("`Cohort` level from Y and X without covariates works fine", {
  ...
  W <- data.frame(adopt_date = adopt_date, W, check.names = FALSE)
  X <- data.frame(adopt_date = adopt_date, popwt = rep(1, 10))
  panel <- list(Y = Y_df, W = W, X = X, never_treat = "2500")
  panel_avg <- prepare_matrices(panel)
...
test_that("`Cohort` level from Y_wt without covariates works fine", {
  ...
  panel <- list(Y_wt = Y_wt, W = W, never_treat = "2500")
  panel_avg <- prepare_matrices(panel)
...
# TODO: Write 4 `cohort` level tests: (contr_cov, contr_cov with treat_cov) x (Y with X, Y_wt)
# TODO: Write 2 `unit` level tests: (Y with X, Y_wt)
